...
from code.cnn.cnn_model import CNN_Model
...
model.load_state_dict(torch.load('cnn_model.pth')['net'])
model = model.cuda()
prediction = test_model(model, x).astype(int)
output = pd.DataFrame()
output['prediction'] = prediction
output.to_csv('../../data/output/cnn_prediction.csv', encoding='utf-8', index=False)
