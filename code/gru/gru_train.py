...
from codes.gru.gru_model import GRU_Model
from codes.tokenize import preprocess_text
from codes.word2vec import word2vec
from codes.utils.utils import Config
...
def train_model(model,x, x_len, y, loss_fn):
    x_train = x
    x_len_train = x_len
    y_train = y
    train_dataset = data.TensorDataset(x_train, x_len_train, y_train)
    for epoch in range(model.config.num_epochs):
        ...
        for i, x_y_data in enumerate(train_loader):
            x_train, x_len_train, y_train = x_y_data
            out = model(x_train, x_len_train)
...
model = model.cuda()
train_model(model, x, x_len, y, custom_loss)
